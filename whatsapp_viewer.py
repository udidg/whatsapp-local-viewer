...
VIEWER_JS_URL = (
    "https://cdn.jsdelivr.net/gh/udidg/whatsapp-local-viewer@main/viewer.js"
)
...
def build_html(
    ...
    """
    Build a lean HTML shell that:
    - embeds all message data as a JSON blob
    - loads viewer.js from GitHub (always up-to-date)
    - calls WA.init() once the script is ready
    """
...
window.__WA_CONFIG__ = {{
  ...
  chatName:  {{json.dumps(chat_name)}},
  mediaBase: {{json.dumps(media_dir_rel)}},
  getMediaUrl: null
}};
</script>
<script src="{VIEWER_JS_URL}"
        onload="WA.init(window.__WA_CONFIG__)"
        onerror="document.body.innerHTML='<p style=\\'padding:2rem;font-family:sans-serif\\'>Failed to load viewer.js. Check your internet connection and refresh.</p>'">
</script>
</body>
</html>
"""
