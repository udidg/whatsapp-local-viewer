...
```bash
curl -fsSL https://raw.githubusercontent.com/udidg/whatsapp-local-viewer/main/whatsapp_viewer.py \
  -o /tmp/whatsapp_viewer.py && python3 /tmp/whatsapp_viewer.py "WhatsApp Chat - My Group.zip"
```

This fetches the single script to `/tmp` and runs immediately. The output is written next to your zip as always.
